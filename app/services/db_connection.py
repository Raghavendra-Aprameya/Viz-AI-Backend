...
async def create_database_connection(
    ...
    if data.connection_string:
        ...
        encoded_password = quote_plus(parsed_url.password or "")
        ...
        if db_type == "postgres":
            connection_string = (
                f"postgresql://{username}:{quote_plus(password)}@{host}/{db_name}"
            )
        elif db_type == "mysql":
            connection_string = (
                f"mysql+pymysql://{username}:{quote_plus(password)}@{host}/{db_name}"
            )
        else:
