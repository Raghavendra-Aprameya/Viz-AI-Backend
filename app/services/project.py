...
This module contains functions to manage projects, roles, dashboards,
and permissions in the application.
It includes functions to create, update, delete, and list projects,
roles, and dashboards.
It also includes functions to manage permissions and blacklists.
...
async def create_project(
    ...
    try:
        ...
        project_name = (
            db.query(ProjectModel).filter(ProjectModel.name == project.name).first()
        )
        if project_name:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        user_project_role = UserProjectRoleModel(
            user_id=user_id, project_id=new_project.id, role_id=role_id
        )
...
async def get_projects(
    ...
    """
    Get all projects visible to the current user with detailed statistics.
    ...
    - Team members count
    - Created at in relative hours format
    """
    try:
        ...
        if user.is_super:
            ...
            user_project_roles = (
                ...
                .filter(UserProjectRoleModel.user_id == user_id)
                .all()
            )
            project_ids = [role.project_id for role in user_project_roles]
            project_list = (
                db.query(ProjectModel).filter(ProjectModel.id.in_(project_ids)).all()
            )

        # Build projects list with embedded owners and statistics
        for project in project_list:
            # Get owners for this project
            owners = (
                db.query(UserProjectRoleModel, UserModel)
            ...
            for role, user in owners:
                owner_list.append({"id": str(user.id), "username": user.username})

            # Count active dashboards in this project
            active_dashboards_count = db.query(DashboardModel).filter(
                DashboardModel.project_id == project.id
            ).count()
            
            # Count database connections in this project
            database_connections_count = db.query(DatabaseConnectionModel).filter(
                DatabaseConnectionModel.project_id == project.id
            ).count()
            
            # Count team members in this project
            team_members_count = db.query(UserProjectRoleModel).filter(
                UserProjectRoleModel.project_id == project.id
            ).distinct(UserProjectRoleModel.user_id).count()
            ...
            if hours_ago < 1:
                time_ago_str = "Less than 1 hour ago"
            elif hours_ago == 1:
                time_ago_str = "1 hour ago"
            elif hours_ago < 24:
            ...
            # Add project with embedded owners and statistics to the result
            projects.append(
                {
                    "id": str(project.id),
                    "name": project.name,
                    ...
                    "database_connections": database_connections_count,
...
async def list_all_roles_project(
    ...
    Returns each role's permissions and blacklisted tables.
    """
    try:
        ...
        roles = (
            db.query(RoleModel)
            .filter(
                (RoleModel.project_id == project_id) | (RoleModel.is_global is True)
            )
            .all()
        )

        # If no roles found, return empty list rather than throwing an exception
        if not roles:
            return {"message": "No roles found for this project", "roles": []}

        roles_list = []

        # Consider using a join to get all data in fewer queries
        for role in roles:
            ...
            permission_types = [permission.type for permission in permissions]

            # Get blacklisted tables for the role
            blacklisted_tables = (
                db.query(RoleTableNameModel, ConnectionTableNameModel)
                .join(
                    ConnectionTableNameModel,
                    RoleTableNameModel.table_name_id == ConnectionTableNameModel.id,
                )
                .filter(RoleTableNameModel.role_id == role.id)
                .all()
            )

            blacklist_table_names = [
                {
                    "table_name": table_info[1].table_name,
                    "table_id": table_info[0].table_name_id,
                }
                for table_info in blacklisted_tables
            ]
            ...
            roles_list.append(
                {
                    ...
                    "blacklist": blacklist_table_names,
                }
            )

        return {"message": "Roles retrieved successfully", "roles": roles_list}
...
async def create_dashboard(
    ...
    try:
        ...
        if dashboard_title:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Dashboard title already exists",
            )
        ...
        user_dashboard = UserDashboardModel(
            user_id=user_id, dashboard_id=new_dashboard.id
        )
        ...
        return {"message": "Dashboard created successfully", "dashboard": new_dashboard}
...
async def list_users_all_dashboard(
    ...
    Returns:
        List[dict]: A list of dashboard details such as id, title,
        description, project_id, and created_by.
    ...
    try:
        ...
        results = (
            ...
            .join(
                UserDashboardModel, UserDashboardModel.dashboard_id == DashboardModel.id
            )
            .filter(
...
async def delete_dashboard(
    ...
    try:
        ...
        db.query(UserDashboardModel).filter(
...
async def update_project(
    ...
    Returns:
        dict: A message indicating the successful update of the project,
        along with updated project data.
...
async def update_dashboard(
    ...
    Returns:
        dict: A message indicating the successful update of the dashboard,
        along with updated dashboard data.

    Raises:
        HTTPException: If the dashboard is not found or if there are any
    ...
    try:
        ...
        return {"message": "Dashboard updated successfully", "dashboard": dashboard}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=str(e)
        ) from e
...
async def get_project_owner_service(project_id: UUID, db: Session = Depends(get_db)):
    """
    ...
    Returns:
        dict: A message indicating the successful retrieval of project owners
        and a list of owner usernames and user IDs.
    ...
    try:
        ...
        if not owners:
            return {"message": "No owners found for this project", "owners": []}

        owner_list = []
        for owner in owners:
            owner_user = (
                db.query(UserModel).filter(UserModel.id == owner.user_id).first()
            )
            if owner_user:
                owner_list.append(
                    {"username": owner_user.username, "user_id": str(owner.user_id)}
                )

        return {"message": "Project owner retrieved successfully", "owners": owner_list}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=str(e)
        ) from e
...
async def get_dashboard_owner_service(
    ...
    Returns:
        dict: A message indicating the successful retrieval of dashboard owners
        and a list of owner usernames and user IDs.

    Raises:
        HTTPException: If the dashboard is not found or if there are no owners
        for the dashboard.
    """
    try:
        dashboard = (
        ...
        owners = (
            db.query(UserDashboardModel)
            .filter(
                UserDashboardModel.dashboard_id == dashboard_id,
                UserDashboardModel.is_owner is True,
            )
            .all()
        )

        if not owners:
            return {"message": "No owners found for this dashboard", "owners": []}
...
async def blacklist_service(
    project_id, data: BlackListTableNameRequest, db: Session, token_payload: dict
):
    """
    Add table names to the blacklist for a specific role.
    Args:
        project_id (UUID): The ID of the project.
        data (BlackListTableNameRequest): The request data containing role ID and table names.
        db (Session): The database session.
        token_payload (dict): The decoded JWT payload containing user details.
    Returns:
        dict: A message indicating the successful addition of table names to the blacklist.
    Raises:
        HTTPException: If the user is not authorized, or if the role is not found,
        or if there are any errors during the operation.
    """
    try:
        user_id = UUID(token_payload.get("sub"))
        if not user_id:
        ...
        role = (
            db.query(RoleModel)
            .filter(RoleModel.id == data.role_id, RoleModel.project_id == project_id)
            .first()
        )
        if not role:
            raise HTTPException(
        ...
        for table_name in data.table_name:
            already_exists_table_name = (
                db.query(RoleTableNameModel)
                .filter(
                    RoleTableNameModel.table_name_id == table_name,
            ...
            if already_exists_table_name:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Table name {table_name} already exists in blacklist",
                )
            blacklist = RoleTableNameModel(
                role_id=data.role_id, table_name_id=table_name
            )
            db.add(blacklist)
        ...
        return {"message": "Table names added to blacklist successfully"}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=str(e)
        ) from e
...
async def update_blacklist_service(
    project_id, data: BlackListTableNameRequest, db: Session, token_payload: dict
):
    """
    Update the blacklist for a specific role by replacing existing entries.
    Args:
        project_id (UUID): The ID of the project.
        data (BlackListTableNameRequest): The request data containing role ID and table names.
        db (Session): The database session.
        token_payload (dict): The decoded JWT payload containing user details.
    Returns:
        dict: A message indicating the successful update of the blacklist.
    Raises:
        HTTPException: If the user is not authorized, or if the role is not found,
        or if there are any errors during the operation.
    """
    try:
        ...
        db.query(RoleTableNameModel).filter(
            RoleTableNameModel.role_id == data.role_id
        ).delete()
        # Store all created blacklist entries
        blacklist_entries = []
        ...
        for entry in blacklist_entries:
            db.refresh(entry)
        return {"message": "Table names updated in blacklist successfully"}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=str(e)
        ) from e
...
async def read_data_service(data: ReadDataRequest, db: Session, token_payload: dict):
    ...
    Args:
        data (ReadDataRequest): The request data containing connection ID and query.
        db (Session): The database session.
        token_payload (dict): The decoded JWT payload containing user details.
    Returns:
        dict: A message indicating the successful reading of data.
    Raises:
        HTTPException: If the user is not authorized, or if the database connection
        is not found, or if there are any errors during the operation.
    """
    try:
        # 1. Validate user_id
        user_id = UUID(token_payload.get("sub"))
